...
def ode_f(t, x):
    return np.array([3.0 * x[0] + 2.0 * x[1] + 4.0 * np.exp(5.0 * t),
                     x[0] + 2 * x[1]])


def ode_exact_solution(t):
    return np.array([np.exp(t) + 2.0 * np.exp(4.0 * t) + 3.0 * np.exp(5.0 * t),
                     -np.exp(t) + np.exp(4.0 * t) + np.exp(5.0 * t)])
