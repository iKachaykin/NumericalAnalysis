...
def ode_f(t, x):
    return np.array([x[1] + 2 * np.exp(t), x[0] + t*t])


def ode_exact_solution(t):
    return np.array([np.exp(t) + np.exp(-t) + t * np.exp(t) - t * t - 2.0,
                     np.exp(t) - np.exp(-t) + (t - 1.0) * np.exp(t) - 2.0 * t])
